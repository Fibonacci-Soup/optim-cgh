...
def main():
    ...
    default_cycler = (cycler(color=['r', 'g', 'b', 'k', 'r', 'g', 'b', 'k']) + cycler(linestyle=["--","-.",":","-","--","-.",":","-"]) + cycler(marker=[None,None,None,None,"|","|","|","|"]))
    plt.rc('axes', prop_cycle=default_cycler)
    ## Carry out GS with SS for reference
    time_start = time.time()
    nmse_lists_GS, _ = gerchberg_saxton_3d_sequential_slicing(target_fields, distances, iteration_number=NUM_ITERATIONS, weighting=0, quickest=not RECORD_ALL_NMSE)
    ...
    time_start = time.time()
    nmse_lists_DCGS, _ = gerchberg_saxton_3d_sequential_slicing(target_fields, distances, iteration_number=NUM_ITERATIONS, weighting=0.01, quickest=not RECORD_ALL_NMSE)
    time_elapsed = time.time() - time_start
    ...
    time_start = time.time()
    hologram, nmse_lists_GD_MSE, _ = lbfgs_cgh_3d(
        target_fields,
        distances,
    ...
    time_start = time.time()
    hologram, nmse_lists_GD_RE, _ = lbfgs_cgh_3d(
        target_fields,
        distances,
        sequential_slicing=SEQUENTIAL_SLICING,
        save_progress=False,
    ...
    time_start = time.time()
    hologram, nmse_lists_LBFGS_MSE, _ = lbfgs_cgh_3d(
        target_fields,
        distances,
        sequential_slicing=SEQUENTIAL_SLICING,
        save_progress=False,
    ...
    time_start = time.time()
    hologram, nmse_lists_LBFGS_RE, _ = lbfgs_cgh_3d(
        target_fields,
        distances,
        sequential_slicing=SEQUENTIAL_SLICING,
        save_progress=False,
    ...
    default_cycler = (cycler(color=['0.5', '0.5', 'm', 'b', 'g', 'r']) + cycler(linestyle=["--",":","-.","-",":","-"]) + cycler(marker=[None,None,None,"+","x",None]))
    plt.rc('axes', prop_cycle=default_cycler)
    ...
    plt.plot(np.std(nmse_lists_GS, axis=0), label="GS")
    plt.plot(np.std(nmse_lists_DCGS, axis=0), label="DCGS")
    plt.plot(np.std(nmse_lists_GD_MSE, axis=0), label="GD_MSE")
    plt.plot(np.std(nmse_lists_GD_RE, axis=0), label="GD_RE")
    plt.plot(np.std(nmse_lists_LBFGS_MSE, axis=0), label="LBFGS_MSE")
    plt.plot(np.std(nmse_lists_LBFGS_RE, axis=0), label="LBFGS_RE")
    plt.xlabel("iterarion(s)")
