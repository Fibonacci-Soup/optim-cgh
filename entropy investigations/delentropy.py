...
with open('delentropy_results.csv', 'w', newline='') as output_file:
    file_writer = csv.writer(output_file)
    file_writer.writerow(['image_filename', 'image_delentropy'])

    for i, image_filename in zip( range(len(images)), images ):
        label = image_filename
        image = np.array(ImageOps.grayscale(Image.open(os.path.join(IMAGE_DIRECTORY, image_filename))))
        ...
        # ax[3].imshow( delDensity  , cmap=plt.cm.gray, vmin = 0, interpolation='nearest', origin='lower',
